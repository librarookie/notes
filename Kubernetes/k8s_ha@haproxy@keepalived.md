...
#1. 生成初始化配置文件
kubeadm config print init-defaults > $HOME/kube-home/ha-kubeadm-config.yaml
...
#2.2 指定集群CgroupDriver（1.22及以上默认值systemd，只要容器运行时的 cgroup 驱动也配置为systemd，则可省略此配置）
#2.3 指定集群kube-proxy 的 mode 字段（默认值是 iptables）
...
# 创建新token，有效期可以设置长一些（默认24小时，可使用 --ttl 24h 配置）
...
sudo kubeadm init phase upload-certs --upload-certs && sudo kubeadm token create --print-join-command
```
